...
function MazesTab({showToast}:{showToast:(t:TT)=>void}){
  ...
  return (
    <div style={{maxWidth:760}}>
      ...
      {/* ── IMAGE UPLOAD ── */}
      {step==='idle'&&(
        <Card title="📸 Subir Imagen del Reporte">
          <div style={{border:`2px dashed ${BORDER}`,borderRadius:10,padding:'20px',textAlign:'center',marginBottom:12,cursor:'pointer',background:imagePreview?'transparent':DEEP}}
            onClick={()=>fileRef.current?.click()}>
            {imagePreview
              ? <img src={imagePreview} alt="preview" style={{maxWidth:'100%',maxHeight:300,borderRadius:8,objectFit:'contain'}}/>
              : <div>
                  <div style={{fontSize:32,marginBottom:8}}>📷</div>
                  <p style={{fontFamily:'Cinzel,serif',fontSize:10,color:'#666',textTransform:'uppercase',letterSpacing:'0.1em'}}>Toca para seleccionar imagen</p>
                  <p style={{fontFamily:'Rajdhani,sans-serif',fontSize:12,color:'#444',marginTop:4}}>Screenshot del chat de WhatsApp o del juego</p>
                </div>
            }
          </div>
