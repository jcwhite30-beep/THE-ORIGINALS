...
function MazesTab({showToast}:{showToast:(t:TT)=>void}){
  ...
  const share=calcPointShare(5,ap,ep,parsed.length)

  function parseReport(text:string){
    return text.split(/\n/).map(l=>l.trim()).filter(Boolean).map(line=>{
      const isSupport=line.includes('*')
      const clean=line.replace(/\*/g,'')
      const m=clean.match(/^([A-Za-z0-9_()\s]+?)[\s:\-]+(\d+(?:\.\d+)?)\s*(?:pts?)?$/i)
      return m?{name:m[1].trim(),points:parseFloat(m[2]),isSupport}:null
    }).filter(Boolean) as {name:string;points:number;isSupport:boolean}[]
  }

  async function handleOCR(){
    if(!imageFile)return;setOcrBusy(true)
    try{const t=await runOCR(imageFile);setRawText(t);setParsed(parseReport(t));showToast({msg:'OCR OK',type:'ok'})}
    catch{showToast({msg:'OCR falló — usa texto',type:'err'})}
    finally{setOcrBusy(false)}
  }

  async function handleSubmit(){
    if(!parsed.length){showToast({msg:'Sin datos',type:'warn'});return}
    setBusy(true)
    try{
      const session=await createMazeSession({maze_type:mazeType,total_points:5,admin_points:ap,event_points:ep,session_date:sessionDate,raw_report:rawText})
      let notFound=0
      for(const entry of parsed){
        ...
        if(pl?.length){
          ...
          await supabase.from('maze_attendance').upsert({session_id:session.id,player_id:pl[0].id,attended:true,points_earned:entry.points,is_support:entry.isSupport})
        } else {
      ...
      showToast({msg:`Sesión guardada. ${notFound>0?notFound+' no encontrados.':''}`,type:'ok'})
  ...
  async function loadAlerts(){
    const a=await getPendingAlerts();setAlerts(a)
      ...
      <Card title="Configuración">
        <div style={{display:'flex',gap:8,marginBottom:14}}>
          {(['BD','FV'] as MazeType[]).map(t=>(
            <button key={t} onClick={()=>setMazeType(t)} style={{flex:1,padding:'9px',borderRadius:8,border:`1px solid ${mazeType===t?(t==='BD'?'#e05050':'#4ab8f0'):BORDER}`,background:mazeType===t?`${t==='BD'?'#e05050':'#4ab8f0'}18`:'transparent',color:mazeType===t?(t==='BD'?'#e05050':'#4ab8f0'):'#666',cursor:'pointer',fontFamily:'Cinzel,serif',fontSize:10,textTransform:'uppercase',letterSpacing:'0.1em'}}>
              {t==='BD'?'🐉 BD':'❄️ FV'}
            </button>
          ))}
        </div>
        ...
        {parsed.length>0&&(
          <div style={{background:DEEP,border:`1px solid ${BORDER}`,borderRadius:8,padding:'9px 12px',marginTop:8}}>
            <span style={{fontFamily:'Rajdhani,sans-serif',color:'#888',fontSize:13}}>Pts/participante: </span>
            <span style={{fontFamily:'Cinzel,serif',fontWeight:700,fontSize:18,color:G}}>{share}</span>
            <span style={{fontFamily:'Rajdhani,sans-serif',color:'#555',fontSize:12,marginLeft:8}}>({parsed.length} — {parsed.filter(p=>p.isSupport).length} ★ apoyos)</span>
          </div>
        )}
      </Card>

      {/* OCR */}
      <Card title="📷 Foto del Reporte">
        <input ref={fileRef} type="file" accept="image/*" onChange={e=>setImageFile(e.target.files?.[0]??null)} style={{fontSize:13,color:'#888',marginBottom:10,display:'block',width:'100%'}}/>
        {imageFile&&<Btn onClick={handleOCR} disabled={ocrBusy}>{ocrBusy?'Procesando...':'Leer con OCR'}</Btn>}
      </Card>

      {/* Paste */}
      <Card title="📋 Texto del Reporte">
        <p style={{fontFamily:'Rajdhani,sans-serif',fontSize:12,color:'#666',marginBottom:8}}>Formato: NombrePJ 3 / Morgan* 2 (★ apoyo mágico)</p>
        ...
        <div style={{marginTop:8}}>
          <Btn onClick={()=>setParsed(parseReport(rawText))}>Parsear</Btn>
        </div>
      </Card>

      {parsed.length>0&&(
        <Card title={`Vista previa — ${parsed.length} jugadores`} color={`${G}40`}>
          <div style={{maxHeight:200,overflowY:'auto',marginBottom:12}}>
            {parsed.map((e,i)=>(
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:i<parsed.length-1?`1px solid #0f0f20`:'none'}}>
                <span style={{fontFamily:'Rajdhani,sans-serif',color:'#e8e0d0',fontSize:14}}>{e.name}{e.isSupport&&<span style={{color:'#f0a020',marginLeft:4}}>★</span>}</span>
                <span style={{fontFamily:'Cinzel,serif',fontWeight:700,color:G,fontSize:13}}>{e.points} pts</span>
              </div>
            ))}
          </div>
          <Btn onClick={handleSubmit} disabled={busy} bg='gold'>{busy?'Guardando...':'✓ Guardar Sesión'}</Btn>
        </Card>
      )}

      {/* Alerts */}
      {alerts.length>0&&(
        <Card title={`⚠ Nombres no encontrados — ${alerts.length}`} color='#a0600040'>
          {alerts.map(al=>(
            <div key={al.id} style={{marginBottom:12,paddingBottom:12,borderBottom:`1px solid #1a1a3a`}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:6}}>
                {(sugg[al.id]??[]).map((s:any)=>(
                  <button key={s.name} onClick={()=>resolveAlert(al.id,'link',s.name).then(loadAlerts)}
                ...
                <Btn size='sm' onClick={()=>createPlayer(al.raw_name).then(()=>resolveAlert(al.id,'create')).then(loadAlerts)} bg='gold'>+ Crear PJ</Btn>
              </div>
            </div>
          ))}
        </Card>
